...
export function TileFormModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
    ...
    const mutation = useMutation({
        mutationFn: async (data: InsertTile) => {
            await apiRequest("POST", "/api/tiles", data);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/tiles"] });
            onOpenChange(false);
            form.reset();
            toast({ title: "Tile added successfully" });
        },
    });
